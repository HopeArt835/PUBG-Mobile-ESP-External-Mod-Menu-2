#ifndef tggod_doom_IMPORTANT_ESP_H
#define tggod_doom_IMPORTANT_ESP_H
#include "struct.h"
...
public:
    ...
    void DrawText3(Color color, float stroke,const char *txt, Vec2 pos, float size) {
        if (isValid()) {
            jclass canvasView = _env->GetObjectClass(_cvsView);
    ...
    void DrawHorizontalHealthBar(Vec2 screenPos, float width, float maxHealth, float currentHealth) {
        screenPos -= Vec2(0.0f, 8.0f);
        DrawBox(Color(0, 0, 0, 255), 3, Rect(screenPos.x, screenPos.y, width + 2, 5.0f));
        screenPos += Vec2(1.0f, 1.0f);
...
void ESP::DrawRect(Color color, float thickness, Vec2 start, Vec2 end) {
    if (isValid()) {
        jclass canvasView = _env->GetObjectClass(_cvsView);
